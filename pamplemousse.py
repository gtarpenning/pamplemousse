...
def _create_tomato_icon(fraction: float, outline_only: bool = False) -> NSImage:
    s = 18
...
# -- Break overlay (red tint + large countdown + mouse punishment) ------------
...
class BreakOverlay:
    ...
    def _tick(self):
        if not self._active:
            return
        if time.time() < self._punished_until:
            return
        if self._label:
...
class PomodoroApp(rumps.App):
    ...
    def _update_tomato_icon(self):
        ...
        if self.seconds_left <= 60:
            ...
            outline_only = self._blink_state
        image = _create_tomato_icon(fraction, outline_only)
        try:
